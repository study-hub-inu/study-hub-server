...
//    }
    ...
    @GetMapping("")
    public ResponseEntity<Slice<FindPostResponseByString>> findPostByAllString(@RequestParam(required = false) String title, @RequestParam(required = false) MajorType major, @RequestParam SortByType sortBy, @RequestParam int page, @RequestParam int size) {
        return ResponseEntity.ok(studyPostService.findPostResponseByString(title, major, sortBy, page, size));
    }
